...
class MovieRepositoryImpl(
    ...
    override fun getTrendingMovies(): Flow<List<Movie>> =
        movieDao.getMovies().map { entities -> entities.map { it.toDomain() } }

    override fun getTopRatedMovies(): Flow<List<Movie>> =
        movieDao.getMovies().map { entities ->
            entities
    ...
    override fun syncMovies(): Flow<SyncStatus> = flow {
        ...
        try {
            ...
            val firstError = listOf(popularResult, topRatedResult)
            ...
            val movies = buildList {
                (popularResult as? Result.Success)?.data?.let(::addAll)
                (topRatedResult as? Result.Success)?.data?.let(::addAll)
            }

            movieDao.upsertMovies(movies.distinctBy { it.id }.map { it.toEntity() })
